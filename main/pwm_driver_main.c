...
static const char *TAG = "pwm_uart_communication"; 
// onboard LED GPIO
#define LED_GPIO 2  
#define BUTTON_PIN 4
...
//     // gpio_set_level(BUTTON_PIN,button_state);    
// } 
void button_init(void) 
{ 
...
{ 
    ...
    //    xQueueSendFromISR(button_dealy, &gpio_num, NULL);   
    if(button_task_handle == NULL)
{
   ...
   vTaskNotifyGiveFromISR(button_task_handle, NULL); 
    ...
    // ESP_LOGI(TAG," ye button ISR_SETUP \r\n");  
} 
...
// without interrupt working with commad an sbutton freertos  begin
    // while (1) 
    // {
    ...
    //         {   
    //          vTaskDelay(button_dealy);//2 tick dealy for debounce
    ...
    //                 ledc_set_duty(PWM_MODE, PWM_CHANNEL, duty);   
    //                   ledc_update_duty(PWM_MODE, PWM_CHANNEL);  
    //                   ESP_LOGI(TAG,"button pressed! pwm duty off &LED is OFF"); 
    //                    previous = current;  
    //                  }  
    ...
    //         // ledec_set_duty(PWM_MODE, PWM_CHANNEL, 0);    
         
    //         // gpio_set_level(LED_GPIO, button_state); 
    //         // ESP_LOGI(TAG, "Button Pressed! LED is %s", button_state ? "ON" : "OFF");
    //     }  
    //        else 
    //                  {  
    //                     duty = 50; 
    //                     pwm_run_state= true; 
    //                     current = gpio_get_level(BUTTON_PIN);   
    //                       ESP_LOGI(TAG,"button Released! pwm duty on &LED is ON");  
    //                       ledc_set_duty(PWM_MODE, PWM_CHANNEL, duty);   
    //                   ledc_update_duty(PWM_MODE, PWM_CHANNEL); 
    //                    previous = 1; 
    //                  }
...
// ~~~~~~~~~~~~~~```````````````````````free rtos setup without uart and interrupt ``````````````````````//
// static void pwm_wave_generat(void *pvParameters)
// {
//     int duty = 0;
...
//         // Decrease brightness
//         for (duty = PWM_DUTY_MAX; duty >= 0; duty -= 100)
//         {
//             ledc_set_duty(PWM_MODE, PWM_CHANNEL, duty);
...
//    ESP_LOGI(TAG, "pwm and  uart base code start ==========================================");
    ...
    // xTaskCreate(pwm_wave_generat, "pwm_wave_generator", 2048, NULL, 5, NULL);    
    ...
    // button_task_handle this  point to this  button task and addres of tbuton task in systme 
...
// ``````````````````````wihtout freertos,uart,isr,onrmal while loop``````````````````//
    // while (1)
    // {
    //     // Increase brightness
    //     for (duty = 0; duty <= PWM_DUTY_MAX; duty += 100)
